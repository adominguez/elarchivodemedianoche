...
for(const [table,ids] of [
  ['case_nodes',case001.nodes.map(n=>n.id)],['clues',case001.clues.map(c=>c.id)],
  ['suspects',case001.suspects.map(s=>s.id)],['suspect_facts',case001.suspects.flatMap(s=>s.facts.map(f=>f.id))],
] as const) {
  const survivors=new Set(ids);
  if((snapshot[table]??[]).some(r=>{const row=r as {case_id:string;id:string};return row.case_id===case001.id&&!survivors.has(row.id);})) throw new Error(`La edición elimina registros de ${table}; revisar antes de aplicar`);
}
...
const path=new URL(`../.data/backups/before-case-001-${Date.now()}.json`,import.meta.url);
